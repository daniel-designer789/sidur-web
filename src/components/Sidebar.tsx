...
export default function Sidebar() {
  ...
  return (
    <aside className={`h-full bg-white dark:bg-gray-800 shadow-lg border-${direction === 'rtl' ? 'l' : 'r'} border-gray-200 dark:border-gray-700 flex flex-col`}>
      <div className="p-4 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center space-x-2 rtl:space-x-reverse">
          ...
          <h1 className="text-xl font-bold text-gray-900 dark:text-white truncate">
            {language === 'he' ? 'מערכת ניהול' : 'Management System'}
          </h1>
        </div>
      </div>
        ...
        <ul className="space-y-2">
          {items.map((item) => (
            <li key={item.href}>
              ...
              >
                <span className={`${pathname === item.href ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'} ${direction === 'rtl' ? 'ml-3' : 'mr-3'}`}>
                  {item.icon}
                </span>
                {item.label}
              </Link>
